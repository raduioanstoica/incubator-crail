...
for src in $source_dirs; do
    ...
    for j in $jars; do
        fn=$(basename $j)
        echo "$fn	$j" >> $dest_dir/all_jars_unsorted
    done
done

cat $dest_dir/all_jars_unsorted | sort > $dest_dir/all_jars_sorted
...
while read line; do
    ...
    if [ "$name" == "$pname" ]; then 
        if [ "$md5" != "$pmd5" ]; then
            ...
            if [ "$?" != "0" ]; then
                echo "#$i -- Jar contents mismatch: $libname ($ppath $path). Reason:"
                #cat $dest_dir/out
                i=$(($i+1))
            fi
        fi
